...
class MUDI_probUNet(Dataset):
    ...
    def __init__(
        ...
        super(MUDI_probUNet).__init__(); self.settings = settings; self.random = random
        ...
        self.mask = MUDI_probUNet.get_mask(self.settings, subject[0])           # Patient-Specific Mask Download
        self.data = unmask(self.data[self.idxv, :].T, self.mask).get_fdata().T  # Patient-Specific Image Unmasking
        self.data = MUDI_probUNet.zero_padding(self.data, self.settings.img_shape)  # Image Pre-Processing: Zero Padding
    ...
    def __len__(self):
        if self.target_combo == 0: return self.s_target * self.h_target
        else: return self.target_combo
    ...
    def __getitem__(self, idx):# -> tuple[np.ndarray, np.float32]:
        ...
        if self.random:
            ...
            if self.target_combo != 0:
                ...
                idxh_target = idx % self.h_target                    # Batch's Vertical Index for X_train
                idxv_slice = idx // self.h_target                  # Batch's Horizontal Index for Loop Combo
        ...
        # Merged Input Generation
        y_target_rep = torch.Tensor(y_target).unsqueeze(dim = 1).unsqueeze(dim = 1)
        y_target_rep = y_target_rep.repeat(1, self.settings.img_shape, self.settings.img_shape)
        input_ = torch.cat((torch.Tensor(X_train), y_target_rep), dim = 0)
        ...
        return {'X_train': np.array(X_train).astype(np.float32), 'y_target': np.ravel(y_target).astype(np.float32),
                ...
                'idxv_slice': idxv_slice, 'slice_target': self.idxv_slice[idxv_slice], 'input': input_,
    ...
    def get_data(
        ...
        mask = MUDI_probUNet.get_mask(settings, patient_id)                          # Patient-Specific Mask Download
        ...
        if idxh_source is not None:
            ...
            X_source = MUDI_probUNet.zero_padding(X_source, settings.img_shape)            
        else: X_source = None
        ...
        if idxh_target is not None:
            ...
            X_target = MUDI_probUNet.zero_padding(X_target, settings.img_shape)   
        else: X_target = None
