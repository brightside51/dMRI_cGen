...
class ProbabilisticUnet(nn.Module):
    ...
    def forward(self, X_train, X_target, training=True):
        """
        ...
        if training:
            if self.flow:
                self.log_det_j, self.z0, self.z, self.posterior_latent_space = self.posterior.forward(X_train, X_target)
            else:
                _, self.posterior_latent_space = self.posterior.forward(X_train,X_target)
                self.z = self.posterior_latent_space.rsample()
                self.z0 = self.z.clone()
        _, self.prior_latent_space = self.prior.forward(X_train)
        self.unet_features = self.unet.forward(X_train)
    ...
    def elbo(self, target, mask=None,use_mask = True, analytic_kl=True, reconstruct_posterior_mean=False, mc_samples=1, loss_mode='l2'):
        """
        ...
        if loss_mode == 'l2':
            ...
            raise ValueError('{} is an invalid loss mode'.format(loss_mode))

        reconstruction_loss = criterion(input=self.reconstruction, target=target)

        # Make loss values outside the brain 0
        #reconstruction_loss = reconstruction_loss*mask
