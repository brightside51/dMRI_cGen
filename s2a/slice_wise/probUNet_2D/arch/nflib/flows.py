...
class CondActNorm(nn.Module):
    ...
    def forward(self, x, s, t):
        eps = 1e-6
        z = x * s + t
        log_det = torch.sum( torch.log(torch.abs(s)+eps), dim=1)
        return z, log_det
...
class CondInvertible1x1Conv(nn.Module):
    ...
    def forward(self, x, W):
        eps=1e-6
        z = torch.bmm(x.unsqueeze(1), W).squeeze(1)
        log_det = torch.log(eps+torch.abs(torch.det(W)))
    ...
    def backward(self, z):
        ...
        x = z @ W_inv
        log_det = -torch.sum(torch.log(torch.abs(torch.det(W_inv))))
        return x, log_det

class CondAffineHalfFlow(nn.Module):
    """
    ...
    - NICE only shifts
    """
    def __init__(self, dim, latent_dim, parity, net_class=MLP, nh=24, scale=True, shift=True):
        super().__init__()
        self.dim = dim
        ...
        if scale:
            self.s_cond = net_class(self.dim , self.dim // 2, nh)
        if shift:
            self.t_cond = net_class(self.dim, self.dim // 2, nh)
        self.condition = nn.Linear(latent_dim, self.dim //2)
        self.condition = self.condition.cuda()
        self.s_cond = self.s_cond.cuda()
        self.t_cond = self.t_cond.cuda()
    ...
    def forward(self, x, h):
        ...
        if self.parity:
            x0, x1 = x1, x0
        h = self.condition(h)
        log_s = self.s_cond(torch.cat((x0,h),dim=1))
        s = torch.exp(log_s)
        t = self.t_cond(torch.cat((x0,h),dim=1))
        z0 = x0 # untouched half
        z1 = s * x1 + t # transform this half as a function of the other
        if self.parity:
            z0, z1 = z1, z0
        z = torch.cat([z0, z1], dim=1)
        log_det = torch.sum(torch.log(s+eps), dim=1)
        return z, log_det
