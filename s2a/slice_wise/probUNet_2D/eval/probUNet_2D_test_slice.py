...
from MUDI_probUNet_slice import MUDI_probUNet
...
def probUNet_2D_test_slice(
    ...
    test_loader = DataLoader(   dataset = test_set, pin_memory = True,
                                ...
                                batch_size = test_set.h_target // 4)#settings.batch_size)
    #mask = MUDI_probUNet.get_mask(settings, settings.test_patient_list[0])
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in test_bar:
            ...
            #X_target = apply_mask(X_target, mask)
            #X_target = unmask(X_target, mask).get_fdata().T
