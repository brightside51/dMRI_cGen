...
wm_logger = TensorBoardLogger(checkpoint_folderpath, f'test/results/{mode}/wm')
gm_logger = TensorBoardLogger(checkpoint_folderpath, f'test/results/{mode}/gm')
csf_logger = TensorBoardLogger(checkpoint_folderpath, f'test/results/{mode}/csf')
...
# Mask Initialization
wm_mask = np.array(nib.load(os.path.join(settings.mask_folderpath, f'p{settings.test_patient_list[0]}_wm.nii.gz')).dataobj).T.astype(int)[0]
gm_mask = np.array(nib.load(os.path.join(settings.mask_folderpath, f'p{settings.test_patient_list[0]}_gm.nii.gz')).dataobj).T.astype(int)
csf_mask = np.array(nib.load(os.path.join(settings.mask_folderpath, f'p{settings.test_patient_list[0]}_csf.nii.gz')).dataobj).T.astype(int)
#wm_mask = np.ma.masked_array(wm_mask == 1, wm_mask); gm_mask = np.ma.masked_array(gm_mask == 1, gm_mask); csf_mask = np.ma.masked_array(csf_mask == 1, csf_mask)

print(img_real.shape); print(img_gen.shape)
print(wm_mask.shape); print(gm_mask.shape)
...
for idx, scan_idx in scan_bar:
    ...
    wm_real = torch.Tensor(np.multiply(img_real, wm_mask)[scan_idx, :, :, :])
    wm_gen = torch.Tensor(np.multiply(img_gen, wm_mask)[scan_idx, :, :, :])
    wm_plot = plt.figure(figsize = (30, 25)); plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
    plt.suptitle(f'Test Patient #{settings.test_patient_list[0]} | White Matter Mask Plotting')
    plt.subplot(2, 2, 1, title = 'Original Scan'); plt.imshow(img_real[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 2, title = 'Generated Scan'); plt.imshow(img_gen[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 3, title = 'Original Mask'); plt.imshow(wm_real[settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 4, title = 'Generated Mask'); plt.imshow(wm_gen[settings.sel_slice, :, :], cmap = plt.cm.binary)
    #plt.subplot(2, 2, 3, title = 'Original Mask'); plt.imshow(img_real[scan_idx, settings.sel_slice, :, :], 'gray', interpolation = 'none')
    #plt.imshow(wm_mask[settings.sel_slice, :, :], 'gray', interpolation = 'none')
    #plt.subplot(2, 2, 4, title = 'Generated Mask'); plt.imshow(img_gen[scan_idx, settings.sel_slice, :, :], 'gray', interpolation = 'none')
    #plt.imshow(wm_mask[settings.sel_slice, :, :], 'gray', interpolation = 'none'); wm_logger.experiment.add_figure(f"Mask Plot", wm_plot, scan_idx)
    
    # White Matter Mask Loss Computation
    wm_mse = mse_criterion(wm_gen, wm_real); wm_mae = mae_criterion(wm_gen, wm_real)
    wm_ssim, _ = ssim(  wm_gen.cpu().numpy().astype(np.float32),
                        wm_real.cpu().numpy().astype(np.float32),
        full = True, data_range = (torch.max(wm_real) - torch.min(wm_real)).cpu().numpy())
    
    # --------------------------------------------------------------------------------------------

    # Grey Matter Mask Application
    gm_real = torch.Tensor(np.multiply(img_real, gm_mask)[scan_idx, :, :, :])
    gm_gen = torch.Tensor(np.multiply(img_gen, gm_mask)[scan_idx, :, :, :])
    gm_plot = plt.figure(figsize = (30, 25)); plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
    plt.suptitle(f'Test Patient #{settings.test_patient_list[0]} | Grey Matter Mask Plotting')
    plt.subplot(2, 2, 1, title = 'Original Scan'); plt.imshow(img_real[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 2, title = 'Generated Scan'); plt.imshow(img_gen[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 3, title = 'Original Mask'); plt.imshow(gm_real[settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 4, title = 'Generated Mask'); plt.imshow(gm_gen[settings.sel_slice, :, :], cmap = plt.cm.binary)

    # Grey Matter Mask Loss Computation
    gm_mse = mse_criterion(gm_gen, gm_real); gm_mae = mae_criterion(gm_gen, gm_real)
    gm_ssim, _ = ssim(  gm_gen.cpu().numpy().astype(np.float32),
                        gm_real.cpu().numpy().astype(np.float32),
        full = True, data_range = (torch.max(gm_real) - torch.min(gm_real)).cpu().numpy())

    # --------------------------------------------------------------------------------------------

    # Grey Matter Mask Application
    csf_real = torch.Tensor(np.multiply(img_real, csf_mask)[scan_idx, :, :, :])
    csf_gen = torch.Tensor(np.multiply(img_gen, csf_mask)[scan_idx, :, :, :])
    csf_plot = plt.figure(figsize = (30, 25)); plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
    plt.suptitle(f'Test Patient #{settings.test_patient_list[0]} | CerebroSpinal Fluid Mask Plotting')
    plt.subplot(2, 2, 1, title = 'Original Scan'); plt.imshow(img_real[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 2, title = 'Generated Scan'); plt.imshow(img_gen[scan_idx, settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 3, title = 'Original Mask'); plt.imshow(csf_real[settings.sel_slice, :, :], cmap = plt.cm.binary)
    plt.subplot(2, 2, 4, title = 'Generated Mask'); plt.imshow(csf_gen[settings.sel_slice, :, :], cmap = plt.cm.binary)

    # Grey Matter Mask Loss Computation
    csf_mse = mse_criterion(csf_gen, csf_real); csf_mae = mae_criterion(csf_gen, csf_real)
    csf_ssim, _ = ssim(  csf_gen.cpu().numpy().astype(np.float32),
                        csf_real.cpu().numpy().astype(np.float32),
        full = True, data_range = (torch.max(csf_real) - torch.min(csf_real)).cpu().numpy())
    
    # --------------------------------------------------------------------------------------------

    # Loss Saving
    if len(test_set.idxh_target) > 1000 and scan_idx >= len(test_set.idxh_target) // 2:
        wm_logger.experiment.add_scalar(f"MSE_Loss", wm_mse, scan_idx)
        wm_logger.experiment.add_scalar(f"MAE_Loss", wm_mae, scan_idx)
        wm_logger.experiment.add_scalar(f"SSIM_Index", wm_ssim, scan_idx)
        gm_logger.experiment.add_scalar(f"MSE_Loss", gm_mse, scan_idx)
        gm_logger.experiment.add_scalar(f"MAE_Loss", gm_mae, scan_idx)
        gm_logger.experiment.add_scalar(f"SSIM_Index", gm_ssim, scan_idx)
        csf_logger.experiment.add_scalar(f"MSE_Loss", csf_mse, scan_idx)
        csf_logger.experiment.add_scalar(f"MAE_Loss", csf_mae, scan_idx)
        csf_logger.experiment.add_scalar(f"SSIM_Index", csf_ssim, scan_idx)
    else:
        wm_logger.experiment.add_scalar(f"MSE Loss", wm_mse, scan_idx)
        wm_logger.experiment.add_scalar(f"MAE Loss", wm_mae, scan_idx)
        wm_logger.experiment.add_scalar(f"SSIM Index", wm_ssim, scan_idx)
        gm_logger.experiment.add_scalar(f"MSE Loss", gm_mse, scan_idx)
        gm_logger.experiment.add_scalar(f"MAE Loss", gm_mae, scan_idx)
        gm_logger.experiment.add_scalar(f"SSIM Index", gm_ssim, scan_idx)
        csf_logger.experiment.add_scalar(f"MSE Loss", csf_mse, scan_idx)
        csf_logger.experiment.add_scalar(f"MAE Loss", csf_mae, scan_idx)
        csf_logger.experiment.add_scalar(f"SSIM Index", csf_ssim, scan_idx)
    wm_logger.experiment.add_figure(f"Mask Results", wm_plot, scan_idx)
    gm_logger.experiment.add_figure(f"Mask Results", gm_plot, scan_idx)
    csf_logger.experiment.add_figure(f"Mask Results", csf_plot, scan_idx)
    # import tensorboard
