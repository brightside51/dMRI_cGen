...
# Voxel-Wise Analysis
voxel_mse_img = torch.mean(voxel_mse_img, dim = 1)
voxel_mae_img = torch.mean(voxel_mae_img, dim = 1)
voxel_ssim_img = torch.mean(voxel_ssim, dim = 1)
print(voxel_mse_img.shape); print(voxel_mae_img.shape); print(voxel_ssim_img.shape)
...
plt.subplot(1, 3, 3, title = 'SSIM Index Heatmap'); plt.imshow(voxel_ssim[0, :, :], cmap = plt.cm.binary)
