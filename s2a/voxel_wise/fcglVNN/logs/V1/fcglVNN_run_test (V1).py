...
class MUDI_fcglVNN(Dataset):
    ...
    def label_gen(self):
        ...
        if self.settings.test_target_param != 0:
            ...
            for i in range(settings.test_target_param): assert(idxh_test[i] not in idxh_target
...
# Full cglVNN Model Testing Class Importing
sys.path.append(settings.script_folderpath)
from fcglVNN_test import fcglVNN_test

# Model Training Method
gc.collect(); torch.cuda.empty_cache()
#tensorboard --logdir V1 --samples_per_plugin images=99999999 --bind_all
test_plot = fcglVNN_test(settings, patient_id = 14)
