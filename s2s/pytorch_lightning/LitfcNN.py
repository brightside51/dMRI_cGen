...
class LitfcNN(pl.LightningModule):
    ...
    def val_dataloader(self):
        ValLoader = MUDI_1D.loader(   Path(f"{self.settings.data_folderpath}"),
                                        version = self.settings.data_version,
                                        set_ = 'Test')
        self.val_batches = len(ValLoader)
        return ValLoader
    ...
    def on_train_epoch_end(self):
        ...
        #self.pX_train_fake = unmask(self.pX_train_fake.T, self.pMask_train).get_fdata().T
        #train_recon_loss = self.criterion(self.pX_train_fake, self.pX_train_img)
        self.train_logger.experiment.add_scalar("Loss", self.train_loss, self.num_epochs)
        
        """# Training Parameter Example Original & Reconstructed Image Subplot
        tt_plot = plt.figure(figsize = (20, 10))
        plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
        plt.subplot(1, 2, 1, title = f'Original Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_train_img[self.sel_train_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        plt.subplot(1, 2, 2, title = f'Reconstructed Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_train_fake[self.sel_train_param, self.sel_slice, :, :], cmap = plt.cm.binary)

        # Validation Parameter Example Original & Reconstructed Image Subplot
        tv_plot = plt.figure(figsize = (20, 10))
        plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
        plt.subplot(1, 2, 1, title = f'Original Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_train_img[self.sel_val_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        plt.subplot(1, 2, 2, title = f'Reconstructed Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_train_fake[self.sel_val_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        ...
        del train_recon_loss, tt_plot, tv_plot, self.pX_train_fake"""
    ...
    # Functionality called upon the Start of Validation Epoch
    def on_validation_epoch_start(self):
        self.val_loss = 0
        self.pX_val_fake = torch.empty((0, self.data.num_params))

    # --------------------------------------------------------------------------------------------

    # Validation Step / Batch Loop 
    def validation_step(self, batch, batch_idx):
        ...
        # Forward Propagation & Loss Computation
        X_fake_batch = self.model(X_train_batch)
        self.pX_val_fake = torch.cat((self.pX_val_fake, X_fake_batch.detach().cpu()), dim = 0)
        loss = self.criterion(X_fake_batch, X_batch)
        del X_batch, X_train_batch, X_val_batch, X_fake_batch
        return loss
    
    # Functionality called upon the End of a Batch Validation Step
    def on_validation_batch_end(self, outputs, batch, batch_idx, dataloader_idx): self.val_loss = self.val_loss + outputs.item()
    ...
    def on_validation_epoch_end(self):
        ...
        self.pX_val_fake = unmask(self.pX_val_fake.T, self.pMask_val).get_fdata().T
        self.log('val_recon_loss', self.val_loss)
        #val_recon_loss = self.criterion(self.pX_val_fake, self.pX_val_img)

        # Training Parameter Example Original & Reconstructed Image Subplot
        vt_plot = plt.figure(figsize = (20, 10))
        plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
        plt.subplot(1, 2, 1, title = f'Original Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_val_img[self.sel_train_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        plt.subplot(1, 2, 2, title = f'Reconstructed Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_val_fake[self.sel_train_param, self.sel_slice, :, :], cmap = plt.cm.binary)

        # Validation Parameter Example Original & Reconstructed Image Subplot
        vv_plot = plt.figure(figsize = (20, 10))
        plt.xticks([]); plt.yticks([]); plt.grid(False); plt.tight_layout()
        plt.subplot(1, 2, 1, title = f'Original Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_val_img[self.sel_val_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        plt.subplot(1, 2, 2, title = f'Reconstructed Image (Parameter #{self.sel_train_param})')
        plt.imshow(self.pX_val_fake[self.sel_val_param, self.sel_slice, :, :], cmap = plt.cm.binary)
        
        # TensorBoard Logger Model Visualizer, Update for Scalar Values & Image Visualizer
        self.val_logger.experiment.add_scalar("Loss", self.val_loss, self.num_epochs)
        ...
        del vt_plot, vv_plot, self.pX_val_fake
